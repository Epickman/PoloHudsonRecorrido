...
// animejs se usa solo para micro-animaciones de UI (entrada del intro, badges,
// contador, botón compartir). El scroll-scrub del mapa sigue siendo de GSAP/ScrollTrigger.
...
// Wrapper de animate() que respeta prefers-reduced-motion: si el usuario lo pidió,
// aplica el estado final al instante (duration/delay 0) en vez de tweenear.
const safeAnimate: typeof animate = (targets, params) =>
  prefersReducedMotion() ? animate(targets, { ...params, duration: 0, delay: 0 }) : animate(targets, params);

// Feedback de "press" en botones (scale down/up). No-op con reduced motion:
// no hay estado final distinto del inicial, así que no hace falta animar nada.
function pressPop(el: HTMLElement | null | undefined) {
  if (!el || prefersReducedMotion()) return;
  animate(el, { scale: [1, 0.92, 1], duration: 260, ease: 'outQuad' });
}
...
// Los primeros `points.length` vértices de route.geojson coinciden 1 a 1 con los
// Los puntos editoriales (mismo relevamiento GPS) — es la porción que se dibuja/
// recorre en sincronía con el scroll de los capítulos. El resto del trazado real
// (el cierre del circuito) sólo se revela en el zoom-out final (`fullRoute`).
const mainRouteCoordinates = routeCoordinates.slice(0, points.length);
...
function MapCamera({ activeIndex, fullRoute, routeProgress }: { activeIndex: number; fullRoute: boolean; routeProgress: number }) {
  ...
  useEffect(() => {
    ...
    const coordinate = mainRouteCoordinates[Math.min(mainRouteCoordinates.length - 1, Math.floor(routeProgress * (mainRouteCoordinates.length - 1)))] ?? [point.lat, point.lng];
...
function LeafletMap({ activeIndex, fullRoute, routeProgress }: { activeIndex: number; fullRoute: boolean; routeProgress: number }) {
  // Mientras se scrollea, la línea activa avanza sobre mainRouteCoordinates
  // (los vértices que coinciden 1 a 1 con los puntos) para que quede siempre
  // a la altura del marcador activo. Recién en el zoom-out final se dibuja
  // el trazado real completo (incluye el cierre del circuito).
  const visibleRoute = fullRoute
    ? routeCoordinates
    : mainRouteCoordinates.slice(0, Math.max(2, Math.ceil(mainRouteCoordinates.length * Math.max(routeProgress, 0.02))));
  ...
  return (
    <MapContainer
      ...
      // Mapa no navegable: la cámara la maneja únicamente el scroll de la página
      // (MapCamera). Se desactiva toda interacción manual de zoom/pan/teclado.
      zoomControl={false}
      dragging={false}
      ...
      keyboard={false}
    >
      {/* Tiles estándar de OSM (gratis, sin API key) + filtro CSS oscuro
          (.leaflet-map img, en index.css). CartoDB Dark Matter ahora exige
          key para uso anónimo y los tiles salían con el watermark "API KEY REQUIRED". */}
      <TileLayer
  ...
  return (
    <figure className="map-stage" id="mapa" data-testid="map-stage">
      ...
      <div className="map-note">
        <strong>TRAZA {routeStatus === 'gps' ? 'RELEVADA EN CAMPO' : routeStatus === 'reference' ? 'DE REFERENCIA' : routeStatus === 'sample' ? 'DE EJEMPLO' : 'EDITORIAL'}</strong><br />
        {routeStatus === 'gps' ? 'Recorrido real, medido con GPS' : 'Basada en captura · no GPS definitivo'}
      </div>
    </figure>
  );
}
...
function App() {
  ...
  const progressStrongRef = useRef<HTMLElement | null>(null);
  const shareLabelRef = useRef<HTMLSpanElement | null>(null);
  // Cachea el split por palabras de cada título: splitText() envuelve el
  // contenido en spans, así que sólo se llama una vez por capítulo aunque
  // se revisite (scroll adelante/atrás) varias veces.
  const headingSplittersRef = useRef(new Map<number, ReturnType<typeof splitText>>());
  const progress = useMemo(() => ((activeIndex + 1) / points.length) * 100, [activeIndex]);

  // Entrada escalonada del intro (animejs), reemplaza el fade-in por CSS estático.
  useEffect(() => {
    if (!introCopyRef.current) return;
    const targets = introCopyRef.current.querySelectorAll('.js-reveal');
    safeAnimate(targets, {
      opacity: [0, 1],
      translateY: [18, 0],
      duration: 800,
  ...
  // Transición de un punto a otro (animejs): pop del número de capítulo y del
  // contador de progreso, línea divisoria que crece, título que entra palabra
  // por palabra con rebote, y descripción/coordenada detrás en cascada.
  useEffect(() => {
    const chapter = chapterRefs.current[activeIndex];
    const badge = chapter?.querySelector('.chapter-index span:first-child');
    if (badge) safeAnimate(badge, { scale: [0.72, 1], opacity: [0.35, 1], duration: 420, ease: 'outBack' });
    if (progressStrongRef.current) safeAnimate(progressStrongRef.current, { scale: [1.35, 1], duration: 380, ease: 'outQuart' });

    const line = chapter?.querySelector('.chapter-index-line');
    if (line) safeAnimate(line, { scaleX: [0, 1], duration: 520, ease: 'outQuart' });

    const heading = chapter?.querySelector<HTMLHeadingElement>('h2');
    if (heading) {
      if (prefersReducedMotion()) {
        heading.style.opacity = '1';
      } else {
        let split = headingSplittersRef.current.get(activeIndex);
        if (!split) {
          split = splitText(heading, { words: { wrap: 'clip' } });
          headingSplittersRef.current.set(activeIndex, split);
        }
        animate(split.words, {
          opacity: [0, 1],
          translateY: ['110%', '0%'],
    ...
    const restTargets = chapter?.querySelectorAll('.chapter-dek, .chapter-coord');
    if (restTargets && restTargets.length) {
      safeAnimate(restTargets, {
        opacity: [0, 1],
        translateX: [-16, 0],
        duration: 560,
        ease: 'outQuart',
  ...
  // Swap animado del texto del botón "Compartir" cuando cambia el label (animejs).
  const isFirstShareRender = useRef(true);
  useEffect(() => {
    if (isFirstShareRender.current) {
      isFirstShareRender.current = false;
      return;
    }
    if (!shareLabelRef.current) return;
    safeAnimate(shareLabelRef.current, { opacity: [0, 1], translateY: [6, 0], duration: 320, ease: 'outQuad' });
  }, [shareLabel]);

  // Reveal escalonado del finale, una sola vez, cuando entra al viewport (animejs).
  useEffect(() => {
    if (!finaleRef.current) return;
    const targets = finaleRef.current.querySelectorAll('.js-reveal');
  ...
  return (
    <div className="field-page">
      ...
      <main>
        <section className="intro" id="inicio" aria-labelledby="intro-title">
          ...
          <div className="intro-copy" ref={introCopyRef}>
            <div className="eyebrow js-reveal">Berazategui, Buenos Aires · hoja 01</div>
            <h1 id="intro-title" className="js-reveal">Entrar al <em>territorio</em></h1>
            <div className="intro-deck js-reveal">
              ...
              <div className="intro-meta mono"><strong>{points.length}</strong> puntos de interés<br />1 ruta editorial<br />datos en revisión</div>
            </div>
            <a className="scroll-cue js-reveal" href="#recorrido" data-testid="link-start-reading">
            ...
            {points.map((point, index) => (
              ...
              >
                <div className="chapter-inner">
                  <div className="chapter-index"><span>{String(point.index).padStart(2, '0')}</span><span>{point.type}</span><span className="chapter-index-line" /></div>
        ...
        <section className="finale" ref={finaleRef} id="cierre" aria-labelledby="finale-title">
          <div className="finale-grid">
            <h2 id="finale-title" className="js-reveal">Mirar el <em>conjunto</em> también es avanzar.</h2>
            <div className="finale-copy">
              <p className="js-reveal">El zoom-out devuelve una lectura completa: un corredor de producción, diseño y movimiento. Esta es una experiencia editorial sobre una geometría todavía abierta.</p>
              <div className="finale-actions js-reveal">
                <button
                  type="button"
                  className="field-button field-button-primary"
                  onClick={(event) => { pressPop(event.currentTarget); jumpTo(0); }}
                ...
                <button
                  ...
                  onClick={(event) => { pressPop(event.currentTarget); downloadRoute(); }}
                  data-testid="button-download-route"
                >
                ...
                <button
                  type="button"
                  className="field-button"
                  onClick={(event) => { pressPop(event.currentTarget); shareJourney(); }}
                  data-testid="button-share-final"
                >
                  <Share2 size={14} aria-hidden="true" /> <span ref={shareLabelRef}>{shareLabel}</span>
                </button>
              </div>
            </div>
          </div>
      ...
      <div className="progress-label" aria-live="polite" data-testid="text-progress">
        parada <strong ref={progressStrongRef}>{String(activeIndex + 1).padStart(2, '0')}</strong> / {String(points.length).padStart(2, '0')}
      </div>
      <div className="route-controls" aria-label="Controles del recorrido">
        <button
          type="button"
          onClick={(event) => { pressPop(event.currentTarget); jumpTo(activeIndex - 1); }}
